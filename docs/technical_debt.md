...
| TD-038 | Multi-tenancy T4-T6 | Complete ownership signatures, same-tenant constraints, owner/grant predicates, two-tenant tests, and forced RLS for Finance, Messaging, Notifications, Gamification, Analytics, and Wellbeing | Scheduling, Workouts, Execution, Feedback, and Coaching are enforced. Enabling RLS before each remaining runtime entry point establishes the right tenant or user context would cause outages, so the tenancy audit fails closed until each loop completes | High | 2026-08-03 |
| TD-039 | Multi-tenant object migration | Migrate existing `invoices/...` and `avatars/...` MinIO objects to `organizations/<organization_id>/...` and `users/<user_id>/avatars/...`, update persisted keys, and verify checksums | New uploads and downloads enforce canonical prefixes, but existing object movement requires an operator-controlled inventory and coordinated database/object-store migration | High | 2026-08-03 |

defmodule MilosTraining.Application.GetAdminUserSchedule do
    ...
    with %{} = user <- Identity.find_by_id(user_id) || {:error, :not_found},
      ...
      {:ok,
       %{
         ...
         items: items(user, start_date, end_date)
       }}
    else
  ...
  defp items(%{role: :athlete, id: id}, start_date, end_date) do
    Workouts.list_assigned_workouts_for_athlete(id, start_date, end_date)
    |> Enum.map(&Map.put(&1, :kind, "assignment"))
  end

  defp items(%{role: :member, id: id}, start_date, end_date) do
    start_at = DateTime.new!(start_date, ~T[00:00:00], "Etc/UTC")
    end_at = end_date |> Date.add(1) |> DateTime.new!(~T[00:00:00], "Etc/UTC")

    Scheduling.list_member_slots(id, start_at, end_at)
    |> Enum.map(&Map.put(&1, :kind, "class"))
  end

  defp items(_user, _start_date, _end_date), do: []
end

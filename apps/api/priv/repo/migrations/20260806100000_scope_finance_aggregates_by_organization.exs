defmodule MilosTraining.Repo.Migrations.ScopeFinanceAggregatesByOrganization do
  use Ecto.Migration
  ...
  def up do
    ...
    WITH aggregate_facts AS (
      ...
      GROUP BY m.organization_id, period_start, user_type_snapshot, package_code, package_family
      ...
      SELECT
        ...
        COALESCE(SUM(GREATEST(mp.amount_cents - COALESCE(mpr.reversed_cents, 0), 0)) FILTER (WHERE mp.payment_status = 'paid'), 0)::bigint,
      ...
      LEFT JOIN (
        SELECT membership_payment_id, SUM(amount_cents)::bigint AS reversed_cents
        FROM finance_payment_reversals
        GROUP BY membership_payment_id
      ) mpr ON mpr.membership_payment_id = mp.id
      GROUP BY 1, 2, 3, 4, 5
      ...
      SELECT
        ...
        date_trunc(
          'month',
          COALESCE(original_entry.occurred_at, fcle.occurred_at)
        )::date,
        COALESCE(m.user_type_snapshot, 'unknown'),
        COALESCE(mps.package_code_snapshot, 'unassigned'),
        COALESCE(mps.package_family_snapshot, 'unassigned'),
        ...
        0::integer, 0::integer, 0::integer, 0::integer, 0::integer, 0::bigint,
        0::integer, 0::integer,
        COALESCE(SUM(fcle.amount_cents) FILTER (
          WHERE fcle.amount_cents > 0 AND fcle.source_type != 'reversal'
        ), 0)::bigint,
        ABS(LEAST(COALESCE(SUM(
          CASE
            WHEN fcle.amount_cents < 0 THEN fcle.amount_cents
            WHEN fcle.source_type = 'reversal' THEN fcle.amount_cents
            ELSE 0
          END
        ), 0), 0))::bigint,
        COALESCE(SUM(fcle.amount_cents), 0)::bigint
      FROM finance_credit_ledger_entries fcle
      LEFT JOIN finance_credit_ledger_entries original_entry
        ON original_entry.id = fcle.reversed_credit_ledger_entry_id
      JOIN memberships m ON m.id = fcle.membership_id
      LEFT JOIN membership_payments mp
        ON mp.id = COALESCE(original_entry.membership_payment_id, fcle.membership_payment_id)
      LEFT JOIN membership_package_subscriptions mps ON mps.id = mp.membership_package_subscription_id
      GROUP BY 1, 2, 3, 4, 5
    )
    SELECT
    ...
    WHERE period_start IS NOT NULL AND organization_id IS NOT NULL
    GROUP BY organization_id, period_start, user_type_snapshot, package_code, package_family
    WITH NO DATA
    """)

    execute("""
    CREATE UNIQUE INDEX finance_aggregates_org_period_user_type_package_index
    ON finance_aggregates (organization_id, period_start, user_type_snapshot, package_code, package_family)
    """)
  ...
  def down do
    raise "finance_aggregates organization scoping is not automatically reversible"
  end
end

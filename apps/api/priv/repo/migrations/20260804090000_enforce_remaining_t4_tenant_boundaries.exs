defmodule MilosTraining.Repo.Migrations.EnforceRemainingT4TenantBoundaries do
  use Ecto.Migration
  ...
  def up do
    ...
    execute("""
    CREATE OR REPLACE FUNCTION milos_apply_tenant_context()
    RETURNS trigger
    LANGUAGE plpgsql
    AS $$
    DECLARE
    ...
    BEGIN
      scoped_organization_id := NULLIF(current_setting('app.organization_id', true), '')::uuid;

      IF scoped_organization_id IS NOT NULL THEN
  ...
  def down do
    ...
    execute("DROP FUNCTION IF EXISTS milos_apply_tenant_context()")
  end
